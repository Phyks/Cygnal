...
export function isWithinPolygon(latLng, polygon, ignoreBoundary) {
    let shouldIgnoreBoundary = ignoreBoundary || false;
...
export function pointToPolygonDistance(latLng, polygon) {
    ...
    if (polygonRing[0][0] !== polygonRing[polygonRing.length - 1][0] && polygonRing[0][1] !== polygonRing[polygonRing.length - 1][1]) {
...
export function pointToGeometryDistance(latLng, geometry) {
    lngLatCoordinates = Array.concat([], geometry.coordinates);
